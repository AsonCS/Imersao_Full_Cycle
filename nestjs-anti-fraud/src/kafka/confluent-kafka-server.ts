...
export class ConfluentKafkaServer extends Server implements CustomTransportStrategy {
	...
	constructor(protected readonly options: KafkaServerOptions) {
		...
		this.clientId = (this.options.server?.['client.id'] || KAFKA_DEFAULT_CLIENT) + postfixId
		this.groupId = (this.options.consumer?.groupId || KAFKA_DEFAULT_GROUP) + postfixId
	...
	public getPublisher(
		...
		return (data: any) => this.sendMessage(data, replyTopic, replyPartition, correlationId)
	}
	...
	public assignIsDisposedHeader(outgoingResponse: OutgoingResponse, outgoingMessage: Message) {
	...
	public assignCorrelationIdHeader(correlationId: string, outgoingMessage: Message) {
		//@ts-expect-error - headers exists
		outgoingMessage.headers[KafkaHeaders.CORRELATION_ID] = Buffer.from(correlationId)
	}

	public assignReplyPartition(replyPartition: string, outgoingMessage: Message) {
	...
	public assignErrorHeader(outgoingResponse: OutgoingResponse, outgoingMessage: Message) {
		...
		//@ts-expect-error - headers exists
		outgoingMessage.headers[KafkaHeaders.NEST_ERR] = Buffer.from(stringifiedError)
	}
